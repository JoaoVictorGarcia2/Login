class Login {
    ...
    static login = (callback_ok, callback_naook, config = null) => {
        ...
        "img {width: 90%;border-radius: 0px 10px 10px 0px;}"+
        ".baseLogin {display: flex;justify-content: center;align-items: stretch;width: 50%;box-sizing: inherit;}"+
        ".elementosLogin {display: flex;justify-content: center;align-items: flex-start;flex-direction: column;width: 50%;background-color: rgb(206, 206, 206);padding: 10px;border-radius: 10px 0px 0px 10px;box-sizing: inherit;}"+
        ".logoLogin {display: flex;justify-content: center;align-items: center;width: 50%;background-color: rgb(184, 184, 184);padding: 10px;border-radius: 0px 0px 0px 0px;box-sizing: inherit;}"+
        ...
        `.botoesLogin button {cursor: pointer;background-color:${this.config.cor};color: white;border-radius: 5px;padding: 10px 30px;box-sizing: inherit;}`
        ...
        const labelUsename = document.createElement("label");
        labelUsename.innerHTML = "Username";
        campoLoginUsername.appendChild(labelUsename);

        const inputUsename = document.createElement("input");
        inputUsename.setAttribute("id", "f_usename"); 
        inputUsename.setAttribute("type", "text"); 
        inputUsename.setAttribute("name", "f_usename"); 
        campoLoginUsername.appendChild(inputUsename);
    ...
    static verificaLogin = () => {
        const mat = document.querySelector("#f_usename").value;
        const pas = document.querySelector("#f_senha").value;

        const endpoint = `https://teste2.joaovictorgarc1.repl.co/?matricula=${mat}&senha=${pas}`
        fetch(endpoint)
        .then(res => res.json())
        .then(res => {
            if(res) {
